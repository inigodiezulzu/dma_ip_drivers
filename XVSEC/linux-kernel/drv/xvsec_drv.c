...
#include "xvsec_mcap/xvsec_mcap.h"
...
static int xvsec_gen_open(struct inode *inode, struct file *filep)
{
	...
	mutex_lock(&dev_ctx->lock);//spin_lock(&dev_ctx->lock);
...
CLEANUP:
	mutex_unlock(&dev_ctx->lock);//spin_unlock(&dev_ctx->lock);
	return ret;
}
...
int xvsec_initialize(struct pci_dev *pdev, struct context *dev_ctx)
{
	...
	mutex_init(&dev_ctx->lock);//spin_lock_init(&dev_ctx->lock);
